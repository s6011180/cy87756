...
class bcn_breadcrumb_trail
{
	...
	protected function post_parents($id, $frontpage)
	{
		...
		$parent = get_post($id);
		//Place the breadcrumb in the trail, uses the constructor to set the title, template, and type, get a pointer to it in return
                $title = get_post_meta( $id, 'breadcrumb', 1 ) ? get_post_meta( $id, 'breadcrumb', 1 ) : get_the_title($id);
                if( $id != 30 )
                        $breadcrumb = $this->add(new bcn_breadcrumb($title, $this->opt['Hpost_' . $parent->post_type . '_template'], array('post', 'post-' . $parent->post_type), get_permalink($id), $id));
		//Make sure the id is valid, and that we won't end up spinning in a loop
	...
	protected function do_post()
	{
		...
		//Place the breadcrumb in the trail, uses the bcn_breadcrumb constructor to set the title, template, and type
                $title = get_post_meta( $post->ID, 'breadcrumb', 1 ) ? get_post_meta( $post->ID, 'breadcrumb', 1 ) : get_the_title($post->ID);
		$breadcrumb = $this->add(new bcn_breadcrumb($title, $this->opt['Hpost_' . $post->post_type . '_template_no_anchor'], array('post', 'post-' . $post->post_type, 'current-item'), NULL, $post->ID));
