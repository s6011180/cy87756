...
define('WATERMARK', dirname(__FILE__).'/images/watermark.png');
//dump(WATERMARK);
...
class timthumb {
	...
	public function __construct(){
		...
		$this->src = $this->param('src');
                if( strpos($this->src, 'imgsfldr') !== false )
                    $this->src = str_replace ('imgsfldr', 'wp-content/uploads', $this->src);
...
//		imagealphablending ($canvas, false);
		...
		if ($zoom_crop > 0) {
			...
			imagecopyresampled ($canvas, $image, 0, 0, 0, 0, $new_width, $new_height, $width, $height);
                ...
                // Custom code
//                dump($mimeType);
                if( $new_width > 440 && $new_height > 299 && $mimeType == 'image/jpeg'){
        ...
        protected function addWaterMark( $im, $watermark_path = WATERMARK ){
