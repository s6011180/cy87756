...
function tp_post_type_init() {
  ...
  $args = array(
    ...
    'show_in_menu' => true, 
    'query_var' => true,
    'rewrite' => array( 'slug' => _x( 'doctors', 'URL slug', 'twentytwelve' ) ),
  ...
  $args = array(
    ...
    'show_in_menu' => true, 
    'query_var' => true,
    'rewrite' => array( 'slug' => _x( 'departments', 'URL slug', 'twentytwelve' ) ),
    'capability_type' => 'page',
    'has_archive' => true, 
    'hierarchical' => true,
    'menu_position' => null,
    'supports' => array( 'title', 'editor', 'author', 'comments', 'custom-fields', 'excerpt', 'thumbnail', 'page-attributes' )
  ); 
...
function tb_the_order_form_mini( $post_id ){
    ...
    <div class="title4"><?php echo $contact_title; ?></div>
        <?php echo do_shortcode('[contact-form-7 id="291" title="Заявка на лечение (мини)"]'); ?> 
    </div>
<?php
}
...
function tp_redirect_for_wpcf7()
{
    ...
    wp_enqueue_script( 'contact-form-7-custom',
                    WPCF7_PLUGIN_URL.'/includes/js/wpcf7-redirect.js',
...
function wpc_mail_additional_info( $args ){
        ...
        $request_page = "http://".filter_input( INPUT_SERVER, 'SERVER_NAME').filter_input( INPUT_SERVER, 'REQUEST_URI');
        $referrer = filter_input( INPUT_SERVER, 'SERVER_NAME');
